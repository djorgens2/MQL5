...
void CEvent::SetEvent(EventType Event, AlertType Alert=Notify, double Price=NoValue)
  {
    ...
    event[0].Event          = Event;
    event[0].Alert          = Alert;
    event[0].Price          = BoolToDouble(IsEqual(Price,NoValue),_Close(0),Price);

    ArrayInsert(eLog,event,0);    
  }
...
string CEvent::EventLogStr(void)
  {
    ...
    for (int node=0;node<ArraySize(eLog);node++)
    {
      ...
      Append(text,DoubleToString(eLog[node].Price,_Digits),"@");
    }
